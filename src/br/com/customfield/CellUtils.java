...
class CellUtils {
	...
	static <T> MaskedField createMaskedField(final Cell<T> cell, final StringConverter<T> converter, String mask) {
		return createMaskedField(cell, converter, mask, null);
	}

	static <T> MaskedField createMaskedField(final Cell<T> cell, final StringConverter<T> converter, String mask, EventHandler<KeyEvent> eventHandler) {
		final MaskedField textField = new MaskedField();
		//        textField.setText(getItemText(cell, converter));
		...
		textField.setOnKeyPressed(t-> {
			...
			if(eventHandler != null)eventHandler.handle(t);
...
//		});
	...
	static <T> FormattedField createFormattedField(final Cell<T> cell, final StringConverter<T> converter, FormatBuilder formatter) {
		return createFormattedField(cell, converter, formatter, null);
	}

	static <T> FormattedField createFormattedField(final Cell<T> cell, final StringConverter<T> converter, FormatBuilder formatter,  EventHandler<KeyEvent> eventHandler) {
		final FormattedField textField = new FormattedField(formatter);
		textField.setValue(getItemText(cell, converter));
		textField.setOnKeyPressed(t-> {
			if (t.getCode() == KeyCode.ENTER || t.getCode() == KeyCode.TAB) {
				if (converter == null) {
					throw new IllegalStateException(
